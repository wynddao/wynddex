...
use crate::state::{
    ExchangeConfig, MigrateConfig, MigrateStakersConfig, DESTINATION, EXCHANGE_CONFIG, MIGRATION,
};
...
const CONTRACT_NAME: &str = "crates.io:raw-migration";
...
/// and provides WYND equivalent given by specified exchange rate
    ...
    let new_assets = assets
        ...
        .map(|asset| {
            if asset.info == raw_asset {
                ...
                // now return wynd tokens instead
                Ok(Asset {
                    info: AssetInfo::Token(exchange_config.wynd_token.to_string()),
                    amount: asset.amount * exchange_config.raw_to_wynd_exchange_rate,
                })
            } else {
                Ok(asset.clone())
            }
