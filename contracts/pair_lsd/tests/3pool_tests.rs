...
fn provide_and_withdraw_no_fee() {
    ...
    let assets = vec![
        ...
        helper.assets[&test_coins[2]].with_balance(100_000_000u128),
    ];
    ...
    helper
        .withdraw_liquidity(
            &user2,
            300_000_000,
            vec![helper.assets[&test_coins[0]]
                .with_balance(300_000_000u128)
                .into()],
        )
        .unwrap();
    ...
    assert_eq!(300_000_000, helper.coin_balance(&test_coins[0], &user2));
    ...
    let err = helper
        .withdraw_liquidity(
            &user3,
            100_000_000,
            vec![helper.assets[&test_coins[1]]
                .with_balance(101_000_000u128)
                .into()],
        )
        .unwrap_err();
    assert_eq!(
    ...
    assert_eq!(101_000_000, helper.coin_balance(&test_coins[1], &user3));
...
fn provide_with_different_precision() {
    ...
    for user_name in ["user1", "user2"] {
        ...
        let assets = vec![
            helper.assets[&test_coins[0]].with_balance(1_000_000u128),
            helper.assets[&test_coins[1]].with_balance(10_000_000u128),
            helper.assets[&test_coins[2]].with_balance(100_000_000u128),
        ];
...
fn swap_different_precisions() {
    ...
    // 100 x FOO tokens
    let offer_asset = helper.assets[&test_coins[0]].with_balance(1_000_000u128);
...
fn check_withdraw_charges_fees() {
    ...
    helper
        .withdraw_liquidity(
            ...
            lp_tokens_amount,
            vec![helper.assets[&test_coins[1]]
                .with_balance(usual_swap_amount)
                .into()],
        )
        .unwrap();
...
fn check_5pool_prices() {
    ...
    let assets = vec![
        ...
        helper.assets[&test_coins[1]].with_balance(100_000_000_000_000_u128),
        helper.assets[&test_coins[2]].with_balance(100_000_000_000_000_u128),
        helper.assets[&test_coins[3]].with_balance(100_000_000_000_000_u128),
        helper.assets[&test_coins[4]].with_balance(100_000_000_000_000_u128),
    ];
    helper.provide_liquidity(&owner, &assets).unwrap();
    ...
    let offer_asset = helper.assets[&test_coins[3]].with_balance(10_000_000_000_u128);
    helper.give_me_money(&[offer_asset.clone()], &user1);
    helper
        .swap(
            &user1,
