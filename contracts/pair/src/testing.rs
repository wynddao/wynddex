...
use wyndex::pair::{
    ...
    SimulationResponse, StakeConfig, TWAP_PRECISION,
};
...
fn query_historical() {
    ...
    assert_eq!(
        ...
        vec![
            (
                uusd.clone(),
                token.clone(),
                vec![(env.block.time.seconds(), 1_000_000u128.into())]
            ),
            (
                token.clone(),
                uusd.clone(),
    ...
    // need to set balance manually to simulate funds being sent
    deps.querier
        .with_balance(&[(&MOCK_CONTRACT_ADDR.into(), &[coin(1_001_000u128, "uusd")])]);
    deps.querier.with_token_balances(&[
        (
            &"asset0000".into(),
    ...
    execute(
        deps.as_mut(),
        env.clone(),
        mock_info(user, &[coin(1_000u128, "uusd")]),
        msg,
    )
    .unwrap();
    ...
    .unwrap();
    // swap increased uusd, so uusd price decreases and token price (in uusd) increases
    assert_eq!(
        history.historical_prices,
        vec![
            (
                ...
                token.clone(),
                vec![
                    (env.block.time.seconds() - HALF_HOUR, 1_000_000u128.into()),
                    (env.block.time.seconds(), 1_002_000u128.into())
                ]
            ),
            (
                token,
                ...
                vec![
                    (env.block.time.seconds() - HALF_HOUR, 1_000_000u128.into()),
                    (env.block.time.seconds(), 998_002u128.into())
                ]
            )
        ],
    );
